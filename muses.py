...
'''
	Script for tuning and hyperparam investigating phase
'''
...
def main():
	...
	#-----------------------------------------------------------------------------
	# Testing... try to get familiar with xgboost.
	# Tuning n_estimators
	...
	# file = dirname(sys.argv[0]) + "/results/lr_training_result_all_features.txt"
	# f = open(file, 'w')
	...
	file = dirname(sys.argv[0]) + "/results/XGBoost_result_all_features_lr01.txt"
	f = open(file, 'w')

	scoring = {'Balanced_accuracy': make_scorer(balanced_accuracy_score), 'Accuracy': make_scorer(accuracy_score)}

	estimator = XGBClassifier(learning_rate=0.1, n_estimators=300, max_depth=9, n_jobs=-1,
