...
def main():
	...
	print("Calculate accuracies and confusion matrices for models...")
	print("Fitting XGB...")
	accuracy_xgb, bl_accuracy_xgb = testing_model(X_train, y_train, X_test, y_test, best_XGB, enc, title='Confusion matrix for XGBoost', savefig = dirname(realpath(sys.argv[0])) + "/results/conf_matrix/XGB_with_SMOTE.png")	
	print("Fitting SVM RBF...")
	accuracy_svm_rbf, bl_accuracy_svm_rbf = testing_model(X_train, y_train, X_test, y_test, best_SVM_RBF, enc, title='Confusion matrix for SVM RBF', savefig = dirname(realpath(sys.argv[0])) + "/results/conf_matrix/SVM_RBF_with_SMOTE.png")
	print("Fitting DT...")
	accuracy_dt, bl_accuracy_dt = testing_model(X_train, y_train, X_test, y_test, best_DT, enc, title='Confusion matrix for Decision Tree', savefig = dirname(realpath(sys.argv[0])) + "/results/conf_matrix/decision_tree_with_SMOTE.png")
	print("Fitting NN1...")
	accuracy_nn1, bl_accuracy_nn1 = testing_model(X_train, y_train, X_test, y_test, best_NN1, enc, title='Confusion matrix for Neural Network 3 layers', savefig = dirname(realpath(sys.argv[0])) + "/results/conf_matrix/neural_network1_with_SMOTE.png")
	print("Fitting NN2...")
	accuracy_nn2, bl_accuracy_nn2 = testing_model(X_train, y_train, X_test, y_test, best_NN2, enc, title='Confusion matrix for Neural Network 4 layers', savefig = dirname(realpath(sys.argv[0])) + "/results/conf_matrix/neural_network2_with_SMOTE.png")
	print("Fitting LR...")
	accuracy_lr, bl_accuracy_lr = testing_model(X_train, y_train, X_test, y_test, best_LR, enc, title='Confusion matrix for Logistic Regression', savefig = dirname(realpath(sys.argv[0])) + "/results/conf_matrix/logistic_regression_with_SMOTE.png")
	print("Fitting SVM Linear...")
	accuracy_svm_linear, bl_accuracy_svm_linear = testing_model(X_train, y_train, X_test, y_test, best_SVM_linear, enc, title='Confusion matrix for SVM Linear', savefig = dirname(realpath(sys.argv[0])) + "/results/conf_matrix/svm_linear_with_SMOTE.png")
	print("Fitting ExtraTrees...")
	accuracy_et, bl_accuracy_et = testing_model(X_train, y_train, X_test, y_test, best_ET, enc, title='Confusion matrix for Extra Trees', savefig = dirname(realpath(sys.argv[0])) + "/results/conf_matrix/extratrees_with_SMOTE.png")
